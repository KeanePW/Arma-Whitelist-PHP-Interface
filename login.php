...
/*
Anmeldung mit Forendaten
Zeile 11 Verzeichnis zur index.php vom wbb angeben. Aktuell "../../"
*/
...
class Login {
  ...
  public $errorText = '';
	...
	//Schreiben der Daten aus dem Forum in Session
        include ("config.php");
        $_SESSION['login'] = 1;
        $_SESSION['user_ingame'] = WCF::getUser()->$config_wbb_useroption; //Eingetragener Arma II Name im Forum
        $_SESSION['user_forum'] = WCF::getUser()->username; //wbb Username
        $_SESSION['user_id'] = WCF::getUser()->userID; //wbb User ID
        include ('user.php');
...
<form action="login.php" method="post">
  <p><?echo $lang_login_user?>:<br /><input name="username" type="text" value="<?php echo $this->username ?>" size="30"<?php if ($this->errorField == 'username') {echo ' style="border:2px solid red"'; } ?> /></p>
  <p><?echo $lang_login_password?>:<br /><input name="password" type="password" value="<?php echo $this->password ?>" size="30"<?php if ($this->errorField == 'password') {echo ' style="border:2px solid red"'; } ?> /></p>
  <p><input type="submit" value="<?echo $lang_login_send?>"> <input type="reset" value="<?echo $lang_login_cancel?>"></p>
</form>
